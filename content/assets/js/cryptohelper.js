window.CryptoHelper = function() {	
	...
	this.signString = (certificate, data, toBase64 = true) => {
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				resolve({data: data, sign: signedMessage});
			} catch (e) {
	...
	this.verify = (data, sign, toBase64 = false) => {
		...
		if (data instanceof File) return new Promise((resolve) => {
			const reader = new FileReader();
			...
			reader.onload = () => {
				...
				signedData.propset_Content(fileContent);

				try {
					signedData.VerifyCades(sign, crypto.CADESCOM_CADES_BES, true);
					const signs = signInfo(signedData);
					resolve(signs);
				} catch (e) {
	...
	this.parseCert = (certificate) => {

		const signers = signedData.Signers;
		const count = signers.Count;

		const signs = [];

		for (let i = 1; i <= count; i += 1) {
			const signer = signers.Item(i);
			const certificate = signer.Certificate;

			const sign = {
				ts: signer.SigningTime,
	...
	async function parseCert(signedData) {
		const signers = await signedData.Signers;
		const count = await signers.Count;
