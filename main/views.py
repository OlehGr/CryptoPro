...
from .models import FilesServ, DocServ, FilesDecrypt, FilesCert
...
def cryptopro(request):
    if request.method == 'POST' and request.FILES:
        ...
        FilesCert.objects.create(doc=request.FILES['doc'])
...
def signature(request):
    if request.method == 'POST':
        print(request.POST)
        cert = json.loads(request.POST['cert'])
    ...
    return HttpResponse(f"{MEDIA_URL}signed-{doc}")
...
#
#         return Response({"success": f"Certificate '{doc}' created successfully"})
#
#
